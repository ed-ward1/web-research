...
public class ResearchController {
	...
	/**
	 * Tails a file from a given offset. If the file contains data from the "offset" passed in the
	 * parameter the data is read and returned. If no data is available from the given offset the
	 * method will wait for a number of seconds until data is available. The content is checked
	 * every 500ms. If no data is forthcoming then the method times out, returns the offset as the
	 * current file length and no data is returned.
	 * 
	 * @param tailedFile contains the name of the file an offset from which data should be read
	 * @return a {@link TailedFile} instance holding the data and next offset (the current end of
	 *         file position), which is received by the browser in the form of a JSON string
	 */
	@RequestMapping(path = "/tail", method = RequestMethod.POST)
	public @ResponseBody TailedFile tailPost(@ModelAttribute TailedFile tailedFile) {
		File file = new File(tailedFile.getFileName());
		final int eos = -1;
		FileReader fr = null;
	...
	/**
	 * TODO this method should return a collection of WebPage instances rather than a service
	 * interface
	 * 
	 * @return
	 */
	@RequestMapping(path = "/listWebPages", method = RequestMethod.GET)
	public ModelMap listWebPages() {
